...
public static class glm
{
    ...
    public static float FractalNoise(int octaves, float frequency, float lacunarity, float persistence, Vector2 position)
    {
        ...
        {
            noise += PNoise.Perlin(p.x, p.y) * amplitude;
            p *= lacunarity;
    ...
    public static float Density_Func(Vector3 worldPosition)
    {
        ...
        float noise = FractalNoise(3, 0.5343f, 4.2324f, 0.68324f, new Vector2(worldPosition.x, worldPosition.z));
        float terrain = worldPosition.y - (MAX_HEIGHT * noise);

        float cube = Cuboid(worldPosition, new Vector3(-4.0f, 10.0f, -4.0f), new Vector3(3.0f, 6.0f, 6.0f));
