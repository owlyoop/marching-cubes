...
{
    public class WorleyNoise : VNoise
    {
        ...
        public WorleyNoise(int seed, float frequency, float jitter, float amplitude = 1.0f)
        {
            ...
            Perm = new PermutationTable(64, 255, seed);
        }
        ...
        public float Solid3D(float x, float y, float z)
        {
            x = (x + Offset.x) * Frequency;
            y = (y + Offset.y) * Frequency;
            z = (z + Offset.z) * Frequency;
            ...
            //There are 9 grid points in 9 cells. Iterate through all of them 
            //and find which has the closest point to the uv.
            //We iterate along the 3 middle cells from left to right.

            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    //These are the 3 cells of one column and their permutation number.
                    //Use this number as the hash.
                    /*px = Perm[(int)pX[i], Pi1 - 1];
                    py = Perm[(int)pX[i], Pi1];
                    pz = Perm[(int)pX[i], Pi1 + 1];*/
                    ...
                    //Find the closest cells point to the uv point and use that cells hash.

                    if (d0 < F0) { F0 = d0; H0 = (uint)px; }
                    if (d1 < F0) { F0 = d1; H0 = (uint)py; }
                    if (d2 < F0) { F0 = d2; H0 = (uint)pz; }
                }
            }

            //make number in the -1, 1 range
            return (((int)H0 - 128) * 0.00390625f) * Amplitude;
        }
